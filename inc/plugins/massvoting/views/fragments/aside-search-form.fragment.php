<form action="<?= $form_action ?>" class="search-box" method="GET" autocomplete="off">
    <div class="pos-r">
	 ...
	 <div class="pos-r">
        <input type="hidden" class="input" name="only_active" value="<?= $AuthUser->get("data.massvoting_pro_only_active") ? 1 : 0 ?>">
        <?php 
            $only_active_cv = $AuthUser->get("data.massvoting_pro_only_active"); 
            if ($only_active_cv): 
        ?>
